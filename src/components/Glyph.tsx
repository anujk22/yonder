...
type GlyphName = 'arrow' | 'back' | 'camera' | 'check' | 'chevron' | 'clock' | 'close' | 'eye' | 'lock' | 'map' | 'shield';
...
export function Glyph({ name, color, size = 20, strokeWidth = 1.8 }: { name: GlyphName; color: string; size?: number; strokeWidth?: number }) {
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24">
      ...
      {name === 'eye' && <><Path d="M2.5 12s3.4-5.2 9.5-5.2 9.5 5.2 9.5 5.2-3.4 5.2-9.5 5.2S2.5 12 2.5 12Z" {...common} /><Circle cx="12" cy="12" r="2.4" {...common} /></>}

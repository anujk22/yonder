...
export type YonderStore = {
  ...
  setCaptureMode: (mode: 'demo' | 'device') => void;
  setLocationEvidence: (fix: LocationFix | null) => void;
...
export const createYonderState: StateCreator<YonderStore> = (set, get) => ({
  ...makeInitialState(),
  ...
  createDraftQuery: () => {
    const state = get();
    if (!state.resolvedPlaceId || !state.draftQuestion.trim() || isUnsafeQuestion(state.draftQuestion)) return null;
    if (state.places.find(p => p.id === state.resolvedPlaceId)?.status === 'blocked') return null;
    ...
    set((state) => ({
      queries: state.queries.map((query) =>
        query.id === activeTaskId
          ? { ...query, state: 'OPEN' as const, refundReason: reason, statusLog: [...query.statusLog, { label: 'Observation released', at: Date.now(), detail: reason }] }
          : query,
      ),
      activeTaskId: null,
    }));
